 ...
 * @param {} addPermission - Add comment permission
 */

function CommentsSection({ postId, addPermission }: { postId: number; addPermission: boolean }): JSX.Element {
  const user = getToken();
        ...
        addPermission && (
          <>
            <div className="row px-3 pt-3 pb-2 shadow-sm rounded border c-bg-lighter">
              <h3 className="fw-bold c-tx-dark">Add Comment</h3>
              {user !== null ? (
                <div className="pt-2">
                  <AddComment postId={Number(postId)} comments={comments} setComments={setComments} />
                </div>
              ) : (
                <div className="alert alert-warning">You must be logged in to post a comment.</div>
              )}
        </div>
            <div className="row p-2">
              <h2 className="fw-bold c-tx-dark">Comments</h2>
              <div className="pt-2">
